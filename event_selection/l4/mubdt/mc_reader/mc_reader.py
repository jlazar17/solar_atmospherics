import numpy as np
from .helpers import mc_fname, check_same_len, mc_name
...
class MCReader(object):
    ...
    def thin(self, n):
        thinner = [True if i%n==0 else False for i in range(len(self._mc_data))]
        self.apply_mask(thinner)
    ...
    def apply_mask(self, mask):
        if self._current_mask is not None:
            mask           = np.logical_and(self._current_mask, mask)
