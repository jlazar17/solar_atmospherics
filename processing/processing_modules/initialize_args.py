...
def make_outfile_name(infile):
    ...
    if 'nancy001' in infile:
        outdir    = '/data/user/jvillarreal/solar_atmospherics/processing/data/nancy/i3'
        ...
        outdir  = '/data/user/jvillarreal/solar_atmospherics/processing/data/nancy/h5/'
        fname   = infile.split('/')[-1].replace('i3.zst', 'h5')
        outfile = '%s/%s' % (outdir, fname)
    elif 'CORSIKA' in infile:
        outdir = '/data/user/jvillarreal/solar_atmospherics/processing/data/corsika/i3/'
        fname  = infile.split('/')[-1].replace('bz2', 'zst')
        outfile = '%s/%s' % (outdir, fname)
    elif 'corsika' in infile:
        outdir = '/data/user/jvillarreal/solar_atmospherics/processing/data/corsika/h5/'
        fname  = infile.split('/')[-1].replace('i3.zst', 'h5')
        outfile = '%s/%s' % (outdir, fname)
    elif 'GENIE' in infile:
        outdir = '/data/user/jvillarreal/solar_atmospherics/processing/data/genie/i3/'
        fname  = infile.split('/')[-1]
        outfile = '%s/%s' % (outdir, fname)
    elif 'genie' in infile:
        outdir = '/data/user/jvillarreal/solar_atmospherics/processing/data/genie/h5/'
        fname  = infile.split('/')[-1].replace('i3.zst', 'h5')
        outfile = '%s/%s' % (outdir, fname)
